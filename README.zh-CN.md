...
1. 手机和电脑连接同一个 Wi-Fi
2. iPhone Wi-Fi 代理改为“手动”
3. “服务器”只填 `<电脑IP>`，不要带 `http://`，也不要带 `:8080`
4. “端口”填 `8080`
5. 如果 Safari 打不开 `http://<电脑IP>:8080/cert`，在桌面端点击“放行手机访问”，允许 Windows 防火墙 8080 入站
6. 用 Safari 打开 `http://<电脑IP>:8080/cert`
7. 下载描述文件
8. 进入 `设置` -> `通用` -> `VPN与设备管理` 安装证书
9. 进入 `设置` -> `通用` -> `关于本机` -> `证书信任设置`
10. 开启 `老虎快跑 Local Debugging Root CA` 的完整信任

手机刚接入时建议先保持桌面端 `HTTPS` 按钮关闭，确认百度等网页能正常打开。证书安装并完整信任后，再开启 `HTTPS`，否则 iOS 会因为不信任调试证书而打不开 HTTPS 网站。

打开证书页后，桌面端抓包列表会出现一条来自手机 IP 的证书请求。如果没有出现，说明 iPhone 还没有连到电脑代理，先检查是否同一 Wi-Fi、是否填错服务器/端口、是否连在访客 Wi-Fi 或路由器开启了 AP 隔离。
